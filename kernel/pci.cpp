...
namespace {
    ...
    Error ScanFunction(uint8_t bus, uint8_t device, uint8_t function) {
        ...
        // ベースクラス 0x06, サブクラス 0x04 であればPCI-to-PCIブリッジ
        if (base == 0x06u && sub == 0x04u) {
            auto bus_numbers = ReadBusNumber(bus, device, function);
            uint8_t secondary_bus = (bus_numbers >> 8) & 0xffu;
            // セカンダリバスに対してScanBusを実行
    ...
    Error ScanBus(uint8_t bus) {
        for (uint8_t device = 0; device < 32; ++device) {
            if (ReadVendorId(bus, device, 0) == 0xffffu) {
                // function 0 のベンダIDが無効値なら、デバイス番号の位置に実際のデバイスがない
                continue;
            }
...
namespace pci {
    ...
    uint16_t ReadBusNumbers(uint8_t bus, uint8_t device, uint8_t function) {
        WriteAddress(MakeAddress(bus, device, function, 0x18));
        return ReadData();
    }

    bool IsSingleFunctionDevice(uint8_t header_type) {
        // ヘッダータイプのビット7が1の場合はマルチファンクションデバイス
    ...
    Error ScanAllBus() {
        ...
        // マルチファンクションの場合、ホストブリッジが複数あるので順番に探索する
        for (uint8_t function = 1; function < 8; ++function) {
